...
class SessionManager:
    ...
    async def set_cookie(self):
        try:
            async with self.session.get(self.headURL["oc"], headers=self.headers, timeout=aiohttp.ClientTimeout(total=5)) as response:
    ...
    async def getDataTasks(self, url):
        ...
        try:
            async with retry_client.get(url, headers=self.headers, cookies=self.cookies) as response:
        ...
        try:
            ...
            if cookies:
                async with aiohttp.ClientSession() as self.session:
                    results = await aio.gather(*[self.getDataTasks(url) for url in self.apiURL.values()], return_exceptions=True)
                return results
        except aiohttp.ClientError as e:
            logging.error("Request error: %s", e)
...
async def main():
    ...
    try:
        await session_manager.getAPIdata()
    finally:
